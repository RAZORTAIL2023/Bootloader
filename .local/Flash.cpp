...
bool Bootloader::FlashErase() {
    ...
    EraseInit.Page = this->APP.StartAddr / FLASH_PAGE_SIZE_128_BITS;
    EraseInit.NbPages = this->APP.MAXSize / FLASH_PAGE_SIZE_128_BITS;
    uint32_t PageError = 0x00000000U;
    HAL_StatusTypeDef status = HAL_FLASHEx_Erase(&EraseInit, &PageError);
    if (status == HAL_OK && PageError == 0xFFFFFFFFU) result = true;
